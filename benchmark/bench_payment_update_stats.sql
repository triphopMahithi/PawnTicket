...
DROP PROCEDURE IF EXISTS bench_payment_update_stats$$

CREATE PROCEDURE bench_payment_update_stats (
    IN p_ticket_id   INT,
    IN p_repeat      INT,
    IN p_warmup      INT
)
BEGIN
    ...
    main_loop: WHILE v_i < (p_repeat + p_warmup) DO
        ...
        -- เลือก payment หนึ่งรายการของ ticket นี้
        SELECT payment_ID
        INTO v_payment_id
        FROM Payment
    ...
    IF v_effective > 0 THEN
        SELECT
            'Payment UPDATE only'          AS query_text,
            v_effective                    AS runs,
