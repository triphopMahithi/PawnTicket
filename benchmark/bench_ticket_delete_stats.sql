...
DROP PROCEDURE IF EXISTS bench_ticket_delete_stats$$

CREATE PROCEDURE bench_ticket_delete_stats (
...
BEGIN
    ...
    main_loop: WHILE v_i < (p_repeat + p_warmup) DO
        ...
        -- ถ้าไม่มีอะไรให้ลบแล้ว ออกจาก loop
        ...
        -- ลบ Payment ลูกของ ticket นี้ก่อน (ถ้ามี)
        DELETE FROM Payment
    ...
    IF v_effective > 0 THEN
        SELECT
            'PawnTicket DELETE only'        AS query_text,
            v_effective                     AS runs,
